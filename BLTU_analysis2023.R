...
df1_allcaps <- read_excel("GM EMBL Capture and Telemetry thru 2022.xlsx",1,col_types = "text")

   # don't think I need this one
# df2_all_hsrel <- read_excel("GM EMBL Capture and Telemetry thru 2022.xlsx",3,col_types = "text")
...
keep1 <- c("Notch","JoinID","Date","Year","Sex","Age Class: Adult, Juvenile",
           "Headstart? (Y/N)","IF HEADSTARTED, Hatch Year","Is this an observation of a headstart in captivity?",
...
nyears <- length(allyears)  # 19 years!
...
nhs  <- length(hs_ids)   # 637 unique headstarted turtles in dataset

other_ids <- allIDs[!grepl("HS",allIDs)]
nother <- length(other_ids)   # 88 unique turtles that were not headstarted in the dataset
...
for(i in 1:ninds){
  for(t in firsts[i]:(nyears-1)){
    logit(phi[i,t]) <- phi0.l[age[i,t]] + step(age[i,t]-4)*phi.maleeff*ismale[i] 
  }
}
...
p.hseff ~ dnorm(0,1)  # headstarts may have different detectability even after the first couple years?
...
for(i in 1:ninds){
  for(t in (firsts[i]+1):(nyears)){
    logit(p[i,t]) <- p0.l[age[i,t]] + step(age[i,t]-4)*p.maleeff*ismale[i] +   # effect of being male for subadults and adults
                                      step(age[i,t]-3)*p.hseff*ishs[i] +       # effect of being headstarted after first 2 years
...
saveRDS(mod,file="latestrun3.rds")

# Visualize results -------------------

# mod <- readRDS("latestrun.rds")
# thisid=allIDs[211]
# temp1=subset(df1_allcaps,ID==thisid)
...
jagsUI::densityplot(mod,"phi0[1]")

plot(sims$p0[,3],type="l")
traceplot(mod,"p0[3]")
jagsUI::densityplot(mod,"p0[5]")
...
png("figs/surv_by_age2.png",4,4,units="in",res=300)
errbar(1:4,t1[1:4,2],t1[1:4,3],t1[1:4,1],ylab="Apparent Surv",xaxt="n",xlab="Age class",
       xlim=c(0.5,4.5))
axis(1,at=1:4,labels=c("HS1","HS2","Juv","Sub+Adult"))
dev.off()
...
p3_res <- plogis(qlogis(sims$p0[,3]))  # probability of capturing a resident juvenile
p4_fem_hs <- plogis(qlogis(sims$p0[,4])+sims$p.hseff)
p4_fem_res <- plogis(qlogis(sims$p0[,4]))
p4_male_hs <- plogis(qlogis(sims$p0[,4])+sims$p.hseff+sims$p.maleeff)
p4_male_res <- plogis(qlogis(sims$p0[,4])+sims$p.maleeff)
p5_fem <- plogis(qlogis(sims$p0[,5])+sims$p.adfemeff)
p5_male <- plogis(qlogis(sims$p0[,5])+sims$p.maleeff)

hss <- cbind(p2_hs,p3_hs,p4_fem_hs)  # headstart series
t1 <-  t(apply(hss,2,function(t) quantile(t,c(0.025,0.5,0.975))))

resmales <- cbind(p3_res,p4_male_res,p5_male)  # male series
t2 <-  t(apply(resmales,2,function(t) quantile(t,c(0.025,0.5,0.975))))

resfems <- cbind(p3_res,p4_fem_res,p5_fem)
t3 <-  t(apply(resfems,2,function(t) quantile(t,c(0.025,0.5,0.975))))
  
png("figs/p_by_age2.png",4,4,units="in",res=300)
errbar(2:3-c(0,0.2),t1[1:2,2],t1[1:2,3],t1[1:2,1],ylab="Detection probability",
       xaxt="n",xlab="Age class",ylim=c(0,0.4),xlim=c(0.7,5.5),
       col="blue",errbar.col = "blue")
errbar(3:5,t2[1:3,2],t2[1:3,3],t2[1:3,1],ylim=c(0,0.6),xlim=c(1,5.5),
       col="darkred",errbar.col = "darkred",add=T)
errbar(3:5+0.2,t3[1:3,2],t3[1:3,3],t3[1:3,1],ylim=c(0,0.6),xlim=c(1,5.5),
       col="darkgreen",errbar.col = "darkgreen",add=T)
axis(1,at=1:5,labels=c("HS1","HS2","Juv","Subadult","Adult"))
legend("topleft",bty="n",col=c("blue","darkred","darkgreen"),lty=c(1,1,1),
       pch=20,legend=c("Headstart","Male","Female"))
dev.off()
...
png("figs/N_by_time2.png",5,4,units="in",res=300)
errbar(theseyears-0.2,t1[thisndx,2],t1[thisndx,3],t1[thisndx,1],
       col=gray(0.3),errbar.col = gray(0.3),
       ylab="Abundance",xlab="Time",ylim=c(0,500))
...
legend("topleft",bty="n",pch=c(20,20),col=c(gray(.3),"darkblue"),
...
relyears <- c(2003,2007:2021)+1
releases2 <- c(10,10,20,35,58,28,70,64,60,32,6,16,32,57,56,76)
...
png("figs/Rel_by_time2.png",5,4,units="in",res=300)
errbar(theseyears,t2[thisndx,2],t2[thisndx,3],t2[thisndx,1],
       col="darkgreen",errbar.col = "darkgreen",
       ylab="# Turtles",xlab="Age class",ylim=c(0,700))
# errbar(theseyears+0,t2[thisndx,2],t2[thisndx,3],t2[thisndx,1],
# col="brown",errbar.col = "brown",add=T)
# errbar(theseyears+0.2,t3[thisndx,2],t3[thisndx,3],t3[thisndx,1],
#        col="darkblue",errbar.col = "darkblue",add=T)
lines(theseyears,cumsum(releases[-1]),col="darkblue",lwd=2,lty=2)
legend("topleft",bty="n",pch=c(20,NA),col=c("darkgreen","darkblue"),
